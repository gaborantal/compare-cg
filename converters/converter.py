...
def convert_acg_js(wd):
    ...
    for f in glob.glob(os.path.join(wd, "*" + ext)):
        ...
        for line in lines:
            ...
            src = c[0].strip()
            tgt = c[1].strip()
            add_node(j, ':'.join(src.split(':')[:-2]) + '@' + src.split(':')[-2] if not src.startswith('toplevel') else '[toplevel]', src)
            add_node(j, ':'.join(tgt.split(':')[:-2]) + '@' + tgt.split(':')[-2] if not tgt.startswith('toplevel') else '[toplevel]', tgt)
            add_link(j, src, tgt)
